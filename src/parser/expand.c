...
char *expand_value(char *token, t_shell *mini)
{
    size_t result_size = ft_strlen(token) * 2;  // Initial guess for size
    ...
    while (*tkn_ptr) {
        if (*tkn_ptr == '$' && *(tkn_ptr + 1) && !ft_isspace(*(tkn_ptr + 1))) {
            tkn_ptr++;  // Skip the dollar sign
            char *var_value = extract_env(&tkn_ptr, mini);
            if (var_value) {
                size_t var_length = ft_strlen(var_value);
                if (current_length + var_length >= result_size) {
                    result_size = (current_length + var_length) * 2;
            ...
            if (current_length + 1 >= result_size) {
                result_size *= 2;
                char *new_result = realloc(result, result_size);
                if (!new_result) {
                    free(result);
...
void expand_tokens(t_token *tokens, t_shell *mini)
{
    ...
    while (current && current->value)
    {
        if (current->type != SINGLE_Q && ft_strchr(current->value, '$'))
        {
            char *expanded = expand_value(current->value, mini);
            if (expanded)
            {
                free(current->value);
                current->value = expanded;
            }
            // If expand_value returns NULL, we keep the original value
        }
        current = current->next;
    }
}
